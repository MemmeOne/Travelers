...
<table border="1">
	...
	<tr>
		...
		<td>${mate_dto.hit}</td>
		<td>${mate_dto.ngroup}</td>
		<td>${mate_dto.step}</td> <td>${mate_dto.indent}</td>
	</tr>
	</c:forEach>
		<tr>
		<td colspan="7">
		...
		<c:choose>
			<c:when test="${param.page>1 }">
				<button type="button" onclick="location.href='mate_list_search?page=${param.page-1}'">이전</button>
			</c:when>
			<c:otherwise>
		...
		<c:forEach begin="1" end="${totPage }" step="3" var="cnt">
			...
			<a href="mate_list_search?page=${num }">[${num}]</a>
		</c:forEach>
		
		
		<c:choose>
			<c:when test="${param.page<num}">
				<button type="button" onclick="location.href='mate_list_search?page=${param.page+1}'">다음</button>
			</c:when>
			<c:otherwise>
